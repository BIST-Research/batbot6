...
def main():
    ...
    if not os.path.exists(outfolder):
        os.makedirs(outfolder+'/dynamic')
        os.makedirs(outfolder+'/static')
    ...
    ser2 = serial.Serial()
    ser2.port = TEENSYPORT
    ser2.baudrate = 115200 # arbitrary
    ser2.setRTS(True)
    ser2.setDTR(True)
    ser2.open()
    ...
    while True:
        try:
            if move: time.sleep(0.5)
            if move: write(ser2, [0, 0, 0, 0, 0])
            #if move: time.sleep(0.1)
            ...
            # Record the data
            if move: output_path = datetime.now().strftime(outfolder+'/dynamic/%Y%m%d_%H%M%S.txt')
            if not move: output_path = datetime.now().strftime(outfolder+'/static/%Y%m%d_%H%M%S.txt')
            ...
            with open(output_path, 'w') as f:
                ...
                #title('Left')
                #subplot(2,2,2)
                    plot(distanceArray[20:9900],filt_data2[20:9900])
                    title('Mic')
                    xlabel('Distance of Echo [m]')
                #ylabel('Amplitude')
                #ax3 = subplot(2,2,3)
                ...
                #ax3.set_xlabel('Time [ms]')
                    ax4 = subplot(2,1,2)
                    spec,b,c,d = ax4.specgram(filt_data2[20:9900],Fs=400, NFFT = 1024, noverlap = 1000, cmap = 'jet')
